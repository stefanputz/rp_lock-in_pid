...
int lock_update(rp_app_params_t *params)
{
  ...
  //g_lock_reg->rl_state                  = (int)params[LOCK_RL_STATE                 ].value;
    ...
    g_lock_reg->sf_config                 = (((int)params[LOCK_SF_CFRZI].value) << 6 ) +(((int)params[LOCK_SF_CFRZO].value) << 5 ) +(((int)params[LOCK_SF_BFRZI].value) << 4 ) +(((int)params[LOCK_SF_BFRZO].value) << 3 ) +(((int)params[LOCK_SF_AFRZI].value) << 2 ) + (((int)params[LOCK_SF_AFRZO].value) << 1 ) + ((int)params[LOCK_SF_START].value);
  ...
  //g_lock_reg->ramp_C                    = (int)params[LOCK_RAMP_C                   ].value;
  ...
  //g_lock_reg->cnt_clk2                  = (int)params[LOCK_CNT_CLK2                 ].value;
    ...
    g_lock_reg->pidA_ofs                  = (int)params[LOCK_PIDA_OFS                 ].value;
  ...
  //g_lock_reg->ctrl_A                    = (int)params[LOCK_CTRL_A                   ].value;
    ...
    g_lock_reg->pidB_ofs                  = (int)params[LOCK_PIDB_OFS                 ].value;
  ...
  //g_lock_reg->ctrl_B                    = (int)params[LOCK_CTRL_B                   ].value;
    ...
    g_lock_reg->pidC_ofs                  = (int)params[LOCK_PIDC_OFS                 ].value;
...
int lock_update_main(rp_app_params_t *params)
{
    ...
    params[114].value = (float) ((g_lock_reg->sf_config >> 5 )& 0x01) ; // lock_sf_CfrzO
    params[115].value = (float) ((g_lock_reg->sf_config >> 6 )& 0x01) ; // lock_sf_CfrzI
    ...
    params[141].value = (float)g_lock_reg->ramp_C                ; // lock_ramp_C
    params[142].value = (float)g_lock_reg->ramp_step             ; // lock_ramp_step
    params[143].value = (float)g_lock_reg->ramp_low_lim          ; // lock_ramp_low_lim
    params[144].value = (float)g_lock_reg->ramp_hig_lim          ; // lock_ramp_hig_lim
    params[145].value = (float)g_lock_reg->ramp_reset            ; // lock_ramp_reset
    params[146].value = (float)g_lock_reg->ramp_enable           ; // lock_ramp_enable
    params[147].value = (float)g_lock_reg->ramp_direction        ; // lock_ramp_direction
    params[148].value = (float)g_lock_reg->ramp_B_factor         ; // lock_ramp_B_factor
    params[149].value = (float)g_lock_reg->sin_ref               ; // lock_sin_ref
    params[150].value = (float)g_lock_reg->cos_ref               ; // lock_cos_ref
    params[151].value = (float)g_lock_reg->cos_1f                ; // lock_cos_1f
    params[152].value = (float)g_lock_reg->cos_2f                ; // lock_cos_2f
    params[153].value = (float)g_lock_reg->cos_3f                ; // lock_cos_3f
    params[154].value = (float)g_lock_reg->sq_ref_b              ; // lock_sq_ref_b
    params[155].value = (float)g_lock_reg->sq_quad_b             ; // lock_sq_quad_b
    params[156].value = (float)g_lock_reg->sq_phas_b             ; // lock_sq_phas_b
    params[157].value = (float)g_lock_reg->sq_ref                ; // lock_sq_ref
    params[158].value = (float)g_lock_reg->sq_quad               ; // lock_sq_quad
    params[159].value = (float)g_lock_reg->sq_phas               ; // lock_sq_phas
    params[160].value = (float)g_lock_reg->in1                   ; // lock_in1
    params[161].value = (float)g_lock_reg->in2                   ; // lock_in2
    params[162].value = (float)g_lock_reg->out1                  ; // lock_out1
    params[163].value = (float)g_lock_reg->out2                  ; // lock_out2
    params[164].value = (float)g_lock_reg->slow_out1             ; // lock_slow_out1
    params[165].value = (float)g_lock_reg->slow_out2             ; // lock_slow_out2
    params[166].value = (float)g_lock_reg->slow_out3             ; // lock_slow_out3
    params[167].value = (float)g_lock_reg->slow_out4             ; // lock_slow_out4
    params[168].value = (float)g_lock_reg->oscA                  ; // lock_oscA
    params[169].value = (float)g_lock_reg->oscB                  ; // lock_oscB
    lock_freeze_regs();
    params[170].value = (float)g_lock_reg->X_28                  ; // lock_X
    params[171].value = (float)g_lock_reg->Y_28                  ; // lock_Y
    params[172].value = (float)g_lock_reg->F1_28                 ; // lock_F1
    params[173].value = (float)g_lock_reg->F2_28                 ; // lock_F2
    params[174].value = (float)g_lock_reg->F3_28                 ; // lock_F3
    params[175].value = (float)g_lock_reg->sqX_28                ; // lock_sqX
    params[176].value = (float)g_lock_reg->sqY_28                ; // lock_sqY
    params[177].value = (float)g_lock_reg->sqF_28                ; // lock_sqF
    params[178].value = (float)g_lock_reg->cnt_clk               ; // lock_cnt_clk
    params[179].value = (float)g_lock_reg->cnt_clk2              ; // lock_cnt_clk2
    lock_restore_regs();
    params[180].value = (float)g_lock_reg->read_ctrl             ; // lock_read_ctrl
    params[181].value = (float)g_lock_reg->pidA_sw               ; // lock_pidA_sw
    params[182].value = (float)g_lock_reg->pidA_PSR              ; // lock_pidA_PSR
    params[183].value = (float)g_lock_reg->pidA_ISR              ; // lock_pidA_ISR
    params[184].value = (float)g_lock_reg->pidA_DSR              ; // lock_pidA_DSR
    params[185].value = (float)g_lock_reg->pidA_SAT              ; // lock_pidA_SAT
    params[186].value = (float)g_lock_reg->pidA_sp               ; // lock_pidA_sp
    params[187].value = (float)g_lock_reg->pidA_ofs              ; // lock_pidA_ofs
    params[188].value = (float)g_lock_reg->pidA_kp               ; // lock_pidA_kp
    params[189].value = (float)g_lock_reg->pidA_ki               ; // lock_pidA_ki
    params[190].value = (float)g_lock_reg->pidA_kd               ; // lock_pidA_kd
    params[191].value = (float)g_lock_reg->pidA_in               ; // lock_pidA_in
    params[192].value = (float)g_lock_reg->pidA_out              ; // lock_pidA_out
    params[193].value = (float) ((g_lock_reg->pidA_ctrl                 )& 0x001) ; // lock_pidA_irst
    params[194].value = (float) ((g_lock_reg->pidA_ctrl            >>1  )& 0x001) ; // lock_pidA_freeze
    params[195].value = (float) ((g_lock_reg->pidA_ctrl            >>2  )& 0x001) ; // lock_pidA_ifreeze
    params[196].value = (float)g_lock_reg->ctrl_A                ; // lock_ctrl_A
    params[197].value = (float)g_lock_reg->pidB_sw               ; // lock_pidB_sw
    params[198].value = (float)g_lock_reg->pidB_PSR              ; // lock_pidB_PSR
    params[199].value = (float)g_lock_reg->pidB_ISR              ; // lock_pidB_ISR
    params[200].value = (float)g_lock_reg->pidB_DSR              ; // lock_pidB_DSR
    params[201].value = (float)g_lock_reg->pidB_SAT              ; // lock_pidB_SAT
    params[202].value = (float)g_lock_reg->pidB_sp               ; // lock_pidB_sp
    params[203].value = (float)g_lock_reg->pidB_ofs              ; // lock_pidB_ofs
    params[204].value = (float)g_lock_reg->pidB_kp               ; // lock_pidB_kp
    params[205].value = (float)g_lock_reg->pidB_ki               ; // lock_pidB_ki
    params[206].value = (float)g_lock_reg->pidB_kd               ; // lock_pidB_kd
    params[207].value = (float)g_lock_reg->pidB_in               ; // lock_pidB_in
    params[208].value = (float)g_lock_reg->pidB_out              ; // lock_pidB_out
    params[209].value = (float) ((g_lock_reg->pidB_ctrl                 )& 0x001) ; // lock_pidB_irst
    params[210].value = (float) ((g_lock_reg->pidB_ctrl            >>1  )& 0x001) ; // lock_pidB_freeze
    params[211].value = (float) ((g_lock_reg->pidB_ctrl            >>2  )& 0x001) ; // lock_pidB_ifreeze
    params[212].value = (float)g_lock_reg->ctrl_B                ; // lock_ctrl_B
    params[213].value = (float)g_lock_reg->pidC_sw               ; // lock_pidC_sw
    params[214].value = (float)g_lock_reg->pidC_PSR              ; // lock_pidC_PSR
    params[215].value = (float)g_lock_reg->pidC_ISR              ; // lock_pidC_ISR
    params[216].value = (float)g_lock_reg->pidC_DSR              ; // lock_pidC_DSR
    params[217].value = (float)g_lock_reg->pidC_SAT              ; // lock_pidC_SAT
    params[218].value = (float)g_lock_reg->pidC_sp               ; // lock_pidC_sp
    params[219].value = (float)g_lock_reg->pidC_ofs              ; // lock_pidC_ofs
    params[220].value = (float)g_lock_reg->pidC_kp               ; // lock_pidC_kp
    params[221].value = (float)g_lock_reg->pidC_ki               ; // lock_pidC_ki
    params[222].value = (float)g_lock_reg->pidC_kd               ; // lock_pidC_kd
    params[223].value = (float)g_lock_reg->pidC_in               ; // lock_pidC_in
    params[224].value = (float)g_lock_reg->pidC_out              ; // lock_pidC_out
    params[225].value = (float) ((g_lock_reg->pidC_ctrl                 )& 0x001) ; // lock_pidC_irst
    params[226].value = (float) ((g_lock_reg->pidC_ctrl            >>1  )& 0x001) ; // lock_pidC_freeze
    params[227].value = (float) ((g_lock_reg->pidC_ctrl            >>2  )& 0x001) ; // lock_pidC_ifreeze
    params[228].value = (float)g_lock_reg->ctrl_C                ; // lock_ctrl_C
    params[229].value = (float)g_lock_reg->aux_A                 ; // lock_aux_A
    params[230].value = (float)g_lock_reg->aux_B                 ; // lock_aux_B
    params[231].value = (float) (( g_lock_reg->lock_feedback >> 0 ) & 0x01 ) ; // lock_ctrl_aux_lock_now
    params[232].value = (float) (( g_lock_reg->lock_feedback >> 1 ) & 0x01 ) ; // lock_ctrl_aux_launch_lock_trig
    params[233].value = (float) (( g_lock_reg->lock_feedback >> 2 ) & 0x01 ) ; // lock_ctrl_aux_pidC_enable_ctrl
    params[234].value = (float) (( g_lock_reg->lock_feedback >> 3 ) & 0x01 ) ; // lock_ctrl_aux_pidB_enable_ctrl
    params[235].value = (float) (( g_lock_reg->lock_feedback >> 4 ) & 0x01 ) ; // lock_ctrl_aux_pidA_enable_ctrl
    params[236].value = (float) (( g_lock_reg->lock_feedback >> 5 ) & 0x01 ) ; // lock_ctrl_aux_ramp_enable_ctrl
    params[237].value = (float) (( g_lock_reg->lock_feedback >> 6 ) & 0x01 ) ; // lock_ctrl_aux_set_pidC_enable
    params[238].value = (float) (( g_lock_reg->lock_feedback >> 7 ) & 0x01 ) ; // lock_ctrl_aux_set_pidB_enable
    params[239].value = (float) (( g_lock_reg->lock_feedback >> 8 ) & 0x01 ) ; // lock_ctrl_aux_set_pidA_enable
    params[240].value = (float) (( g_lock_reg->lock_feedback >> 9 ) & 0x01 ) ; // lock_ctrl_aux_set_ramp_enable
    params[241].value = (float) (( g_lock_reg->lock_feedback >>10 ) & 0x03 ) ; // lock_ctrl_aux_trig_type
    params[242].value = (float) (( g_lock_reg->lock_feedback >>12 ) & 0x01 ) ; // lock_ctrl_aux_lock_trig_rise
